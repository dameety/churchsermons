...
class CategoryEditAndDeleteTest extends TestCase
{
    ...
    /** @test */
    public function categoryNameUpdateValidationWorks()
    {
        $name = $this->faker->sentence($nbWords = 1, $variableNbWords = true);
        $request = [
            'name' => $name,
        ...
        $response->assertSessionHasErrors(["name"])->assertJsonMissing(['updated' => true]);
    }

    /** @test */
    public function categoryDescriptionUpdateValidationWorks()
    {
        $name = $this->faker->sentence($nbWords = 1, $variableNbWords = true);
        $request = [
            'name' => $name,
        ...
        $response->assertSessionHasErrors(["description"])->assertJsonMissing(['updated' => true]);
    }
}
